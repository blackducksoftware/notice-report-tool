...
import com.blackducksoftware.sdk.codecenter.request.data.RequestAttachment;
import com.blackducksoftware.sdk.codecenter.request.data.RequestAttachmentToken;
...
public class CCNoticeReportProcessor implements INoticeReportProcessor {
	...
	public enum ATTRIBUTE_COMPONENT_VALUES {LICENSE};
	
	public CCNoticeReportProcessor(NRTConfigurationManager nrtConfigManager, APPLICATION bdsAppType) throws Exception 
	{
		this.nrtConfigManager = nrtConfigManager;
		ccWrapper = new CodeCenterServerWrapper(nrtConfigManager.getServerBean(), nrtConfigManager);
	...
	public HashMap<String, ComponentModel> processProject() 
	{
		...
		try
		{
			...
			for(ComponentModel comp : components)
			{
				...
				{
					log.debug("Excluding component from report: " + comp.getNameAndVersion());
				}
			}
	...
	private List<ComponentModel> getComponentsForApplication(Application app) 
	{
		...
		try
		{
			log.debug("Gathering list of components for application: " + app.getName());
			List<RequestSummary> requests = 
					ccWrapper.getInternalApiWrapper().applicationApi.getApplicationRequests(app.getNameVersion());
			...
			for(RequestSummary request : requests)
			{
				...
				Component comp = 
						cApi.getCatalogComponent(request.getApplicationComponentToken().getComponentId());
	
				
				log.debug("Got component: " + comp.getName() + ":" + comp.getVersion());
				...
				for(LicenseSummary licSummary : licSummaries)
				{
					...
					License license = cApi.getLicense(licSummary.getNameToken());
				...
				nrtCaProcessor.processCustomAttributesForComponent(comp, request.getId(), compModel);
	...
	/**
	 * Looks up the configuration to see whether user specified any rules for custom attributes
	 * If so, then iterate through each component and perform the necessary actions.
	 * @param components
	 */
	private void applyCustomAttributeRules(List<ComponentModel> components) 
	{
		List<CustomAttributeRule> filterAttributeRules = nrtConfigManager.getCustomAttributeRules(ATTRIBUTE_TYPE.FILTER);
		for(CustomAttributeRule filterRule : filterAttributeRules)
		{
			String filterValue = filterRule.getValue();
			for(ComponentModel component : components)
			{
				log.debug("Custom Attribute[filter] for custom attribute application on component: " + component.getNameAndVersion());
				Map<String, CustomAttributeBean> map = component.getAttributeMap();
				String ruleName = filterRule.getName();
				
				CustomAttributeBean bean = map.get(ruleName);
				// If the bean exists, then this component has the requisite settings.
				if(bean != null)
				{
					log.debug("Filter custom attribute exists: " + bean.getName());
					// Now we check if the value of the filter rule, is the same as the value set by the custom attribute
		...
		List<CustomAttributeRule> overrideAttributeRules = nrtConfigManager.getCustomAttributeRules(ATTRIBUTE_TYPE.OVERRIDE);
		for(CustomAttributeRule overrideRule : overrideAttributeRules)
		{
			...
			for(ComponentModel component : components)
			{
				log.debug("Custom attribute[override] for custom attribute application on component: " + component.getNameAndVersion());
				Map<String, CustomAttributeBean> map = component.getAttributeMap();
				CustomAttributeBean bean = map.get(overrideName);
				if(bean != null)
				{
					log.debug("Override custom attribute exists: " + bean.getName());
					...
					if(overrideValue.equals(OVERRIDE_TYPE.LICENSE.toString().toLowerCase()))
					{
						log.debug(overrideValue + " override rule found, applying...");
						// We want to grab the first license, and replace the text with the value from the bean.
						LicenseModel licenseToOverride = component.getLicenses().remove(0);
						if(licenseToOverride != null)
						{
							String appendOverrideString = ATTRIBUTE_TYPE.OVERRIDE.toString();
							String newLicenseText = bean.getValue();
							licenseToOverride.setText(newLicenseText);
							licenseToOverride.setName(licenseToOverride.getName() + "(" + appendOverrideString + ")");
							licenseToOverride.setId(licenseToOverride.getId() + "_" + appendOverrideString);
							component.addNewLicense(licenseToOverride);
							
							log.debug("Overrode license: " + licenseToOverride.getName());
						}
					}
					else
					{
						log.debug("Unrecognized override type: " + overrideValue);
					}
				}
			}
		}		
